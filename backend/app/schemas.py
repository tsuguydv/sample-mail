...
class GenerationResult(BaseModel):
    ...
    isModular: bool = False
    blockLayout: List[dict] = Field(default_factory=list)
    blockContent: dict = Field(default_factory=dict)
...
class ContactListUpload(BaseModel):
    ...
    # Frontend sends the numeric DB list id from JSON; accept int or str.
    @field_validator("list_id", mode="before")
    @classmethod
    def _coerce_list_id(cls, v):
        return str(v) if v is not None else v


class ContactsUploadRequest(BaseModel):
    lists: List[ContactListUpload]


class ContactsUploadResultItem(BaseModel):
    list_id: str
    list_name: str
    total_contacts: int
    uploaded_contacts: int
    batches: int
    errors: List[dict]


class ContactsSendCampaignRequest(BaseModel):
    list_id: str
    list_name: str
...
class ContactsSendCampaignResult(BaseModel):
    single_send_id: str
    status: str
    name: str


class CampaignStatItem(BaseModel):
    id: str
    name: str
    status: str
...
class CampaignStatsResponse(BaseModel):
    campaigns: List[CampaignStatItem]
