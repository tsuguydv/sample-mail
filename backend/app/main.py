...
from app.generator import generate_pack, to_generation_result
...
BUILTIN_TEMPLATE_DEFS = {
    "t1": {
        "name": "template.t1",
        "previewImageUrl": "https://placehold.co/600x400/4f46e5/ffffff?text=Template+1",
        "objectCounts": {"header": 1, "body": 1, "image": 1, "cta": 1, "footer": 1},
        "theme": {"style": "classic", "primary": "#2563eb", "secondary": "#1d4ed8", "accent": "#dbeafe", "surface": "#f8fafc", "colorScheme": "light"},
    },
    "t2": {
        "name": "template.t2",
        "previewImageUrl": "https://placehold.co/600x400/059669/ffffff?text=Template+2",
        "objectCounts": {"header": 1, "body": 2, "image": 1, "cta": 1, "footer": 1},
        "theme": {"style": "minimal", "primary": "#0f766e", "secondary": "#115e59", "accent": "#ccfbf1", "surface": "#f8fafc", "colorScheme": "light"},
    },
    "t3": {
        "name": "template.t3",
        "previewImageUrl": "https://placehold.co/600x400/dc2626/ffffff?text=Template+3",
        "objectCounts": {"header": 1, "body": 3, "image": 1, "cta": 1, "footer": 1},
        "theme": {"style": "bright", "primary": "#0f172a", "secondary": "#334155", "accent": "#e2e8f0", "surface": "#f8fafc", "colorScheme": "light"},
    },
    "t4": {
        "name": "template.t4",
        "previewImageUrl": "https://placehold.co/600x400/8b5cf6/ffffff?text=Template+4",
        "objectCounts": {"header": 1, "body": 1, "image": 0, "cta": 1, "footer": 1},
        "theme": {"style": "modern", "primary": "#4338ca", "secondary": "#3730a3", "accent": "#e0e7ff", "surface": "#f8fafc", "colorScheme": "light"},
    },
...
def _template_def(template_id: str | None, db: Session | None = None, user_id: int | None = None) -> dict:
    ...
    return BUILTIN_TEMPLATE_DEFS.get((template_id or "").strip(), BUILTIN_TEMPLATE_DEFS["t1"])
...
def _send_verification_email(email: str, token: str) -> None:
    ...
    if res.status_code >= 400:
        ...
        try:
            raw = (res.text or "").strip().replace("\n", " ")
            if raw:
                detail = f"{detail}: SendGrid {res.status_code} - {raw[:400]}"
            else:
                detail = f"{detail}: SendGrid {res.status_code}"
        except Exception:
            detail = f"{detail}: SendGrid {res.status_code}"
...
def _sendgrid_sender_is_verified(sender_email: str) -> bool:
    ...
    with httpx.Client(timeout=15.0) as client:
        # Single sender verification list
        s_res = client.get("https://api.sendgrid.com/v3/verified_senders", headers=headers)
        if s_res.status_code < 400:
            senders = (s_res.json() or {}).get("results") or []
            for item in senders:
                em = str(item.get("from_email") or "").lower().strip()
                verified = bool(item.get("verified"))
                if em == sender_email and verified:
                    return True
        # Domain authentication list
        d_res = client.get("https://api.sendgrid.com/v3/whitelabel/domains", headers=headers)
        if d_res.status_code < 400:
            domains = d_res.json() or []
            for dom in domains:
                domain = str(dom.get("domain") or "").lower().strip()
                valid = bool(dom.get("valid"))
                if sender_domain and sender_domain == domain and valid:
                    return True
    return False
...
def api_send_generated_email(email_id: int, payload: dict, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not _sendgrid_sender_is_verified(sender_email):
        raise HTTPException(status_code=400, detail="Sender email is not verified in SendGrid")
...
def api_verify_company_sender(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    verified = _sendgrid_sender_is_verified(sender_email)
