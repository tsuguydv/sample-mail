...
SENDGRID_UNSUBSCRIBE_URL = os.getenv("SENDGRID_UNSUBSCRIBE_URL", "")

# Email-API marketing replacement (contacts/campaigns/analytics without Marketing Campaigns)
SENDGRID_ASM_GROUP_ID = int(os.getenv("SENDGRID_ASM_GROUP_ID", "0")) or None
SENDGRID_EVENT_WEBHOOK_PUBLIC_KEY = os.getenv("SENDGRID_EVENT_WEBHOOK_PUBLIC_KEY")

# Email provider switch: "sendgrid" (default) or "unisender"
EMAIL_PROVIDER = (os.getenv("EMAIL_PROVIDER", "sendgrid") or "").strip().lower()
# UniSender (https://api.unisender.com/ru/api/<method>)
UNISENDER_API_KEY = os.getenv("UNISENDER_API_KEY")
UNISENDER_FROM_EMAIL = os.getenv("UNISENDER_FROM_EMAIL")  # confirmed sender in UniSender
UNISENDER_FROM_NAME = os.getenv("UNISENDER_FROM_NAME", "")
# System list used for the unsubscribe link required by sendEmail
UNISENDER_LIST_ID = os.getenv("UNISENDER_LIST_ID", "")
UNISENDER_BASE_URL = os.getenv("UNISENDER_BASE_URL", "https://api.unisender.com/ru/api")

# LLM provider switch: "gigachat" (default) or "openai"
LLM_PROVIDER = (os.getenv("LLM_PROVIDER", "gigachat") or "").strip().lower()
# GigaChat (Sber)
GIGACHAT_AUTH_KEY = os.getenv("GIGACHAT_AUTH_KEY")  # base64(client_id:client_secret) from the GigaChat project
GIGACHAT_SCOPE = os.getenv("GIGACHAT_SCOPE", "GIGACHAT_API_PERS")  # individual ("физлицо")
...
# Trial individual accounts usually lack the Минцифры root cert; disable verification for testing.
GIGACHAT_VERIFY_SSL = getenv_bool("GIGACHAT_VERIFY_SSL", False)
DISABLE_GENERATION_LIMITS = getenv_bool("DISABLE_GENERATION_LIMITS", False)
